...
for (const workflow of [
  ...
  // This fork publishes nothing to npm, so release.yml has no npm provenance to
  // scope. reusable-release.yml is an orphaned upstream template that nothing
  // invokes; its npm shape is left intact and still asserted here.
  const publishesToNpm = workflow.includes('reusable-release');

  test(`${workflow} never grants id-token at the workflow level`, () => {
  ...
  if (publishesToNpm) {
    ...
    test(`${workflow} does not publish this fork to npm`, () => {
      // CommunityHelp is marked private and has no NPM_TOKEN. A release that
      // tried to publish would either fail loudly or, worse, push under a name
      // someone else owns.
      assert.doesNotMatch(content, /npm publish/);
      assert.doesNotMatch(content, /NPM_TOKEN/);
      assert.doesNotMatch(content, /npm dist-tag add/);
    });

    test(`${workflow} makes no npm registry lookups`, () => {
      // `communityhelp` may exist on npm owned by someone else; a hit would be
      // misread as "this version is already published".
      assert.doesNotMatch(content, /npm view /);
    });

    test(`${workflow} still produces a GitHub Release, the only distribution surface`, () => {
      assert.match(content, /name: Create GitHub Release/);
      assert.match(content, /softprops\/action-gh-release/);
    });
  }
