...
### v3.0.0 — CommunityHelp 分支（2026年9月）

* **AI 素养教练：** 在你提交提示时进行教学，支持西班牙语和英语，并随着你的进步按主题逐个静默。
* **安全性：** 会重写你的文件、从注册表下载工具或将对话记录发送到额外调用的钩子，现在均改为选择性启用。
* **隐私：** 教练绝不会将你的提示文本写入磁盘。请参阅 `PRIVACY.md`。
* **不发布到 npm。** 请从克隆仓库或插件市场安装。
...
>
> 如果你已经通过 `/plugin install` 安装了 ECC，**不要再运行 `./install.sh --profile full`、`.\install.ps1 --profile full` 或 `node scripts/ecc.js install --profile full`**。插件已经会自动加载 ECC 的技能、命令和 hooks；此时再执行完整安装，会把同一批内容再次复制到用户目录，导致技能重复以及运行时行为重复。
>
...
**搞定！** 你现在可以使用 68 个智能体、289 项技能和 99 个命令了。
...
| 命令 | PASS: 99 个    | PASS: 35 个 | **Claude Code 领先** |
